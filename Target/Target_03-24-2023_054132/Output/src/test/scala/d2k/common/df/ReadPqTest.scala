...
result.collect.foreach{ row =>row.getAs[String]("s") mustBe "a"
}
      }
   }
...
Seq(MultiReadTest("c")).toDF.writeParquet("C.pq")
 ...
 val result = mapDf.run(Unit)
result("B.pq").collect.foreach{ row =>row.getAs[String]("s") mustBe "b"
}
result("C.pq").collect.foreach{ row =>row.getAs[String]("s") mustBe "c"
}
      }
   }

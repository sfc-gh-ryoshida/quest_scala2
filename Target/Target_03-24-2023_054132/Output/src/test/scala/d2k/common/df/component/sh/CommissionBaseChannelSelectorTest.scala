...
import com.snowflake.snowpark_extensions.Extensions.functions._
 ...
 class CommissionBaseChannelSelectorTest extends WordSpec with MustMatchers with BeforeAndAfter {
   ...
   def check(key: String, DV_OUTOBJDIV: String, DV_TRICALCOBJDIV: String) = { (df: DataFrame) =>df.filter($"key" === key).collect.foreach{ row =>row.getAs[String]("DV_OUTOBJDIV") mustBe DV_OUTOBJDIV

...
case class LastUpdateTime (ID_TBLID: String, DT_FROMUPDYMDTM: Timestamp, DT_TOUPDYMDTM: Timestamp, DT_UPDYMDTM: Timestamp)
...
NXT_MO_MTH_DT mustBe "20150430"
   ...
   def timestamp_yyyymmdd(yyyy: Int, mm: Int, dd: Int) = Timestamp.valueOf(LocalDateTime.of(yyyy, mm, dd, 0, 0, 0))

   implicit class MakeDate (str: String) {
      def toTm = timestamp_yyyymmdd(str.take(4).toInt, str.drop(4).take(2).toInt, str.drop(6).take(2).toInt)
   }
...
JdbcUtils.createConnectionFactory(readDbInfo.toOptions)().prepareStatement(
"""create table MOP012(
          |  ID_TBLID VARCHAR2(6),
...
"""create table X2(
            ...
            |  TMSTMP TIMESTAMP)
            |""".stripMargin).executeUpdate
(1 to 10).map{ idx =>T1(idx.toString, f"200101${ idx }%02d".toTm)
}.toDF.writeTable("T1")
Seq(
...
"template実装 tableName未定義" in {
 ...
 val dbTodb = new DbToDb with Nothing {
               ...
               override def readDbWhere(inArgs: InputArgs) = Array(s"TMSTMP >= '${ inArgs.lastUpdateTime("xxx") }'")
            }
         }
try

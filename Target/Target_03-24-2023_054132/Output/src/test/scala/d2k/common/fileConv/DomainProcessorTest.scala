...
actual_min_under mustBe min_exp
 ...
 def execLeft(domain_name: String, data: String) = checkLeft(DomainProcessor.exec(domain_name, data))
 def execLeft2(domain_name: String, data: Array[Int], charEnc: String = "MS932") = checkLeft(DomainProcessor.execArrayByte(domain_name, data.map(_.toByte), charEnc))
 def checkLeft(result: Either[String, String]) = if (result.isLeft)
         {
         result.left.get
         }
else
 ...
 def execRight(domain_name: String, data: String) = checkRight(DomainProcessor.exec(domain_name, data))
 def execRight2(domain_name: String, data: Array[Int], charEnc: String = "MS932") = checkRight(DomainProcessor.execArrayByte(domain_name, data.map(_.toByte), charEnc))
 def checkRight(result: Either[String, String]) = if (result.isRight)
         {
         result.right.get
         }
else

...
case class DfCtlData (a: String = "a", b: String = "b", c: String = "c", d: String = "d", e: String = "e", a1: Int = 1, b2: Int = 2)
case class DfCtlCastData (NM_A: String = "100", NM_B: String = "200", STR_C: String = "300", STR_D: String = "400", E: String = "500")
...
r.collect.foreach{ row =>row.getAs[String]("a") mustBe "c"
...
("b" -> "y").r)
 val r = df ~> editColumns(d)
r.collect.foreach{ row =>row.getAs[String]("x") mustBe "a"
...
"b".d)
 val r = df ~> editColumns(d)
r.collect.foreach{ row => val names = row.schema.map(_.name)
names.contains("a") mustBe false
names.contains("b") mustBe false
}
         }
...
r.count mustBe 1
r.collect.foreach{ row =>row.getAs[Int]("a1") mustBe 100
}
      }
   }
...
r.count mustBe 1
r.collect.foreach{ row =>row.getAs[Int]("a1") mustBe 50
}
      }
   }
...
r.count mustBe 1
r.collect.foreach{ row =>row.getAs[Int]("a1") mustBe 101
}
      }
   }
...
checkData(s"${ outPath }/x=c/y=3", "c33.000000000000000000") mustBe true
 ...
 def extentionCheck(path: String, extention: String) = Path(path).jfile.listFiles.map(_.toString.endsWith(extention)).forall(_ == true)
"add Extention" when {
      "single column" in {

...
import com.snowflake.snowpark.expressions.Window
...
import com.snowflake.snowpark_extensions.Extensions.functions._
 ...
 def toCols(colNames: Set[String], cols: Seq[Column]) = (colNames - colName, cols :+ (editor as colName))
   }
 ...
 def toCols(colNames: Set[String], cols: Seq[Column]) = Option(editor).map( e =>(colNames - colNameFrom, cols :+ (editor as colNameTo))).getOrElse((colNames - colNameFrom, cols :+ (col(colNameFrom) as colNameTo)))
   }

   case class Delete (inColName: String) extends Editors {
   val colName = inColName
 def toCols(colNames: Set[String], cols: Seq[Column]) = (colNames - colName, cols)
   }

   case class CastByName (inColName: String, castType: String) extends Editors {
   val colName = inColName
 def toCols(colNames: Set[String], cols: Seq[Column]) = (colNames - colName, cols :+ (col(inColName).cast(castType)))
   }
...
(colNames -- regxMatchNames, cols ++ regxMatchNames.map( name =>col(name).cast(castType)))
   ...
   val colName = inColName
 def toCols(colNames: Set[String], cols: Seq[Column]) = (colNames - colName, cols :+ func(col(inColName)))
   }
...
df.select(cols :_*)
 ...
 def ~|>(f: DataFrame => DataFrame) = {
         df.show(false);f(df)
         }
 def pickMaxValueRow(pks: String*)(maxValueTarget: String*) = df.sort((pks.map( x =>col(x)) ++ maxValueTarget.map( x =>col(x).desc)) :_*).dropDuplicates(pks)
 def partitionWriteFile(filePath: String, overwrite: Boolean = true, charEnc: String = "MS932", partitionExtention: String = "")(func: Row => String) {
            if (overwrite)
               FileCtl.deleteDirectory(filePath)
FileCtl.createDirectory(filePath)
df.rdd.mapPartitionsWithIndex{(idx, iterRow) => val fullPath = FileCtl.addExtention(s"${ filePath }/${ idx }", partitionExtention)
FileCtl.writeToFile(fullPath, true, charEnc){ pw =>elapse(s"fileWrite:${ fullPath }"){
...
Seq.empty[Row].toIterator
 ...
 def partitionWriteToFileWithPartitionColumns(filePath: String, partitionColumns: Seq[String], overwrite: Boolean = true, charEnc: String = "MS932", partitionExtention: String = "")(func: Row => String) {
            if (overwrite)
               FileCtl.deleteDirectory(filePath)
FileCtl.createDirectory(filePath)
df.rdd.mapPartitionsWithIndex{(idx, iterRow) => val fullPath = FileCtl.addExtention(s"${ filePath }/${ idx }", partitionExtention)
elapse(s"fileWrite:${ fullPath }"){
FileCtl.loanPrintWriterCache{ cache =>iterRow.foldLeft(cache){(l, r) =>FileCtl.writeToFileWithPartitionColumns(
...
Seq.empty[Row].toIterator
 ...
 def ~|>(f: (DataFrame, DataFrame) => DataFrame) = {
         df._1.show(false);df._2.show(false);f(df._1, df._2)
         }
      }

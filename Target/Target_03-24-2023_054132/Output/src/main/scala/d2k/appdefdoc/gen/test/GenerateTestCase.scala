...
case class OutputData (groupId: String, appId: String, testCase: String, inputMdData: Seq[(IoMdInfo, String)], outputMdData: Seq[(IoMdInfo, String)]) {
...
writePath.createDirectory(failIfExists = false)
inputMdData.foreach{
      case (ioMd, tableData) => val outPath = writePath / s"${ ioMd.id }.md"
...
ioInfoRegx.findFirstMatchIn(str).map( g =>IoMdInfo(g.group(3).trim, g.group(1).trim, g.group(4).trim, g.group(2).trim))
      }
...
tableTemplate.replaceAllLiterally("%%inputDataName%%", imi.name).replaceAllLiterally("%%inputData%%", s"""x.readMdTable("${ imi.id }.md")""").replaceAllLiterally("%%inputConvMethod%%", ioTypeCnv(imi.ioType, imi.id))
      }
...
s"# ${ imi.name }\n${ MakeResource.itemsMdToTable(s"${ itemsBaseUrl }/${ itemName }").getOrElse("") }\n"
 ...
 val template = Option(getClass.getClassLoader.getResourceAsStream("genTemplates/testcaseAt.tmpl")).map( is =>Source.fromInputStream(is)).get.mkString
 ...
 val testCaseStr = template.replaceAllLiterally("%%APP_NAME%%", appId).replaceAllLiterally("%%PROJECT_ID%%", appGroup).replaceAllLiterally("%%READ_DATA%%", inputInfos.mkString("\n\n")).replaceAllLiterally("%%CHECK_DATA%%", outputInfos.mkString("\n\n"))

...
case class GafData (baseAppId: Option[String], baseResourceId: String, result: Seq[(String, AppDef)])
 object GeneratingApplicationRouteFinder extends App with D2kParser {
   ...
   }.flatMap{ node =>itemNamePath.find{ p =>node._2.physical_name.contains(p._1)
}.map{ p =>s"[${ node._2.physical_name }](${ p._2 })[${ node._2.logical_name }]"
}
...
result, d 
 =>Seq(d.appDetail.map( x =>s"${ x.appDef.appInfo.id } --> ${ d.resDetail.id }_res"), d.parentAppId.map( x =>s"${ d.resDetail.id }_res --> ${ x }")).flatten)

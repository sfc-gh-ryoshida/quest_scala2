...
println(s"[Start Source Generate${ if (isLocalMode)
      " on Local Mode"
else
      "" }] ${ args.mkString(" ") }")
...
inputFiles.headOption.foreach{_ =>generateItemConf(baseUrl, branch, appGroup, appId, writePath)(inputFiles)
}
println(s"[Finish Source Generate] ${ writeFilePath }")
 ...
 def generate(appdef: AppDef) = {
   val componentdefs = appdef.componentList.map{ comp =>println(s"Parsing ${ comp.id }[${ comp.name }]")
if (isLocalMode)
         {
 ...
 val componentStr = componentdefs.map{ c =>println(s"Generating ${ c.componentInfo.id }[${ c.componentInfo.name }]")
 val conved = c.componentDetail.processes.flatMap{ proc =>proc.detail.flatMap{ x =>x.values.flatMap{
      case ProcessParamSubDetail(id, name, defaultCode, value) => Some((defaultCode, value))
      case _ => None
      }.groupBy(_._1).map{ b =>ProcessDetail(x.param, Some(b._1), b._2.map( x =>ProcessParamValue(x._2.mkString)))
}
}
}
...
_.detail.flatMap( p =>ConvertTemplateDefine.modDetail(baseUrl, branch)(c.componentDetail, p)))
 ...
 val impliments = implimentTmpl.replaceAllLiterally("%%functions%%", funcIds).replaceAllLiterally("%%impliments%%", implStr)
...
("%%executor%%", if (c.implimentLogic.impls.isEmpty)
         "Nothing"
else
         "Executor"), 
("%%componentId%%", addDq(c.componentDetail.componentId)), 
("%%procImpliments%%", componentDefs.mkString("\n")), 
("%%impliments%%", if (c.implimentLogic.impls.isEmpty)
         ""
else
         impliments))
componentRepList.foldLeft(componentTmpl){(l, r) =>l.replaceAllLiterally(r._1, r._2)
}
}.mkString("\n\n")
 def defaultAppFlow = appdef.componentList.map{
      case xif !x.id.contains("_Df") => s"c${ x.id }.run(Unit)"
      case x => s"c${ x.id }.run"
      }.mkString(" ~>\n    ")
 val flowLogic = appdef.componentFlow.map{ cf => val flowPair = cf.pairs.map( p =>(p.cf1.componentId, p.cf2.componentId))
...
mainRepList.foldLeft(mainTmpl){(l, r) =>l.replaceAllLiterally(r._1, r._2)
}
   }
...
inputFiles.map{ iodata =>println(s"  Parsing ${ iodata.id }[${ iodata.name }](${ iodata.path })")
 val filePath = iodata.path.split('/').takeRight(2).mkString("/")
 val itemdef = ItemDefParser(baseUrl, branch, filePath).get
 val itemDetails = itemdef.details.map{ d =>Seq(d.id, d.name, d.size, d.dataType, "false", "").mkString("\t")
}

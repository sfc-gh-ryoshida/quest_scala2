...
case class DicData (id: String, name: String, dic: String) {
 ...
 val result = (dbResult ++ pqResult).groupBy( d =>(d.id, d.dic)).map{
      case (k, d) => d.head
      }.toList
 val outputPath = "data/dicGen"
 val outputFile = s"${ outputPath }/d2k_appdef.txt"
Directory(outputPath).createDirectory(true, false)
FileCtl.writeToFile(outputFile, false, "MS932"){ w =>result.sortBy(_.id).foreach(w.println)
}
println(s"[Finish Dictionary Generate] ${ outputFile }")
 ...
 val grpList = md.filter(!_.isEmpty).dropWhile( line =>!line.contains("## 業務グループ一覧")).drop(3).map(_.split('|')(1).trim.split('(')(1).dropRight(1))
...
md.filter(!_.isEmpty).dropWhile( line =>!line.contains("## ")).drop(3).map(_.split('|')(1).trim.split('(')(1).dropRight(1))
   }
...
itemdef.details.map{ items => val target = dicConv.keys.filter( k =>itemdef.id.toLowerCase.startsWith(k)).head
...
md.filter(!_.isEmpty).dropWhile( line =>!line.contains("## 項目変換パターン")).drop(3).map{ x => val splitted = x.split('|')

...
case class AppDef (appInfo: AppInfo, componentFlow: Option[CfData], componentList: Seq[ComponentDefInfo], inputList: Seq[IoData], outputList: Seq[IoData])
...
parsed
   ...
   def 入力データタイトル = コンポーネントフロー <~ ("##" ~ "03. 入出力データ一覧" ~ "###" ~ "03.01. 入力" ~ "|" ~ "定義ID" ~ "|" ~ "ソースタイプ" ~ "|" ~ "定義名" ~ "|" ~ eol ~ tableSep3)

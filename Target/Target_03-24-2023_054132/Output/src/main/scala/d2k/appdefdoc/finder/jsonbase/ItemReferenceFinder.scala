...
import com.snowflake.snowpark_extensions.Extensions.functions._
 object ItemReferenceFinder extends App with D2kParser {
   ...
   println(s"[Start Item Reference Finder${ if (isLocalMode)
      " on Local Mode"
else
      "" }] ${ args.mkString(" ") }")
   ...
   val targetResouces = itemNames.filter{ path =>Try{readItemDefMd(path.toString).contains(targetItemId)}.getOrElse({
   println(s"  itemDef parse error: ${ path }");false
   })
}.toList
   ...
   val rrdList = itemDefList.flatMap{ itemDef => val targetName = itemDef._2.id
...
writeResult.map( x =>(x._1, x._2))
   ...
   val conved = tmpl.replaceAllLiterally("%%SearchTarget%%", targetItemId).replaceAllLiterally("%%ResultResource%%", resultResource.mkString("\n")).replaceAllLiterally("%%ResultItemReference%%", implList.map(_._2).mkString("\n"))
   ...
   case (app, _, subImpl) => val appdef = jsonAppdef.filter(_._2.appInfo.id == app._2.appInfo.id).head
...
localPath, s"${ localPath.dropRight(9) }${ subImpl }").mkString(" , ")
   ...
   case (path, appDef) => Seq(targetItemId, appDef.appInfo.id, appDef.appInfo.name, fcom.localPath2Url(baseUrl, basePath, path)).mkString(" , ")
   }.mkString("\n")

...
writeResult.map( x =>(x._1, x._2))
   ...
   case (app, _, subImpl) => val appdef = appDefList.filter(_._2.appInfo.id == app._2.appInfo.id).head
...
localPath, s"${ localPath.dropRight(9) }${ subImpl }").mkString(" , ")
   ...
   case (path, appDef) => Seq(targetItemId, appDef.appInfo.id, appDef.appInfo.name, localPath2Url(baseUrl, basePath, path)).mkString(" , ")
   }.mkString("\n")

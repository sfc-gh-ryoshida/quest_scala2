...
(splitted(0).trim, splitted(1).trim.dropRight(1))
   ...
   private val inputTypeToPlantUml = (appId: String, indata: (Cf, ComponentDetail)) =>{
   val (cf, cd) = indata
...
(s"""database "${ id }\\n${ name }" as ${ id }""", s"""[抽出] as ${ cf.componentId }""", s"${ id } --> ${ cf.componentId }")
   ...
   private val outputTypeToPlantUml = (appId: String, indata: (Cf, ComponentDetail)) =>{
   val (cf, cd) = indata
...
println(s"[Start Rough Concept Generate${ if (isLocalMode)
      " on Local Mode"
else
      "" }] ${ args.mkString(" ") }")
...
if (!fileCheck)
      new FileWriter (psFilePath).close
...
if (isLocalMode)
 ...
 val objects = appdef.componentFlow.get.pairs.flatMap( x =>Seq(x.cf1, x.cf2)).flatMap{
      case cf:Cf => {
...
Some((cf, targetComponent.head.componentDetail))
 ...
 val links = appdef.componentFlow.get.pairs.flatMap{ pair =>pair.cf2 match {
         case CfEnd => None
         case _ => Some(s"${ pair.cf1.componentId } --> ${ pair.cf2.componentId }")
      }
}

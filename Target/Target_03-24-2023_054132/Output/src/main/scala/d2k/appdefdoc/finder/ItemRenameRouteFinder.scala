...
case class RenameData (componentId: String, beforeName: String, afterName: String)
 ...
 val result = recursiveSearchWithRename(appDefList, itemDefMap, (appdef: AppDef) 
...
renameData.componentId, s"${ rirfData.path.dropRight(9) }${ renameData.componentId }.md", renameData.beforeName, renameData.afterName)
 ...
 val renameAppTableList = renameAppList.map{ x =>mkTable(s"[${ x._1 }](${ x._2 })", x._3, s"[${ x._4 }](${ x._5 })", x._6, x._7)
}.mkString("\n")
...
writer.close
 ...
 val csvReferData = result.map{ data => val resPath = localPath2Url(baseUrl, basePath, data.appData.resDetail.path)
 val csvData = data.appData.appDetail.map{ x => val appUrl = localPath2Url(baseUrl, basePath, x.path)
...
csvReferWriter.close
 ...
 val csvImplData = renameAppList.map{
   case (appid, path, appName, subAppid, subAppPath, beforeName, afterName) => val localAppPath = localPath2Url(baseUrl, basePath, path)

...
import com.snowflake.snowpark_extensions.Extensions.functions._
 class FixedConverter (hasHeader: Boolean = false, hasFooter: Boolean = false, charEnc: String = "MS932", lineBreak: Boolean = true, newLineCode: NewLineCode = LF, withIndex: Boolean = false, preFilter: (Seq[String], Map[String, String] => Boolean) = null, withBinaryRecord: Option[String] = None) extends Serializable {
import Converter._
...
Row(result :_*)
 ...
 def addIndexColumn(names: Seq[String], nameList: Seq[String], domains: Seq[String], sliceLen: List[(Int, Int)]) = (names :+ SYSTEM_COLUMN_NAME.RECORD_INDEX, 
...
sliceLen :+ (-1, -1))
 ...
 val targetData = targetNames.flatMap{ target =>dataAndDomainsAndNames.filter(_._2 == target)
}
...
withBinaryRecord.map(_ =>Row.merge(rowValue, Row(arr.clone))).getOrElse(rowValue)
 ...
 def addStructType(st: StructType) = withBinaryRecord.map( n =>st.add(StructField(n, BinaryType))).getOrElse(st)
 ...
 val rdd = Option(preFilter).map{ pf =>sc.binaryRecords(filePath, totalLen).flatMap{ arr =>if (pf._2(arrToMap(pf._1, names, domains, sliceLen)(arr)))
         Some(arr)
else
         None
}
 ...
 val df = if (withIndex)
         ...
         if (charEnc == "JEF")
               {
               throw new IllegalArgumentException ("JEF CharEnc is not supportted")
               }
...
context.createDataFrame(rddWithIdx.map{
         ...
         {
         context.createDataFrame(rdd.map( arr =>addArr(cnvFromFixed(names, domains, sliceLen)(arr), arr)), addStructType(Converter.makeSchema(nameList)))
         }
...
if (!lineBreak)
         {
         throw new IllegalArgumentException ("must be lineBreak = true when recordLengthCheck = true")
         }
 val (totalLen_, sliceLen) = makeSliceLen(len)
 val totalLen = addLineBreak(totalLen_, lineBreak)
 val ziped = names.zip(domains)
...
Option(preFilter).map{ pf =>resultArr.flatMap{ arr =>if (pf._2(arrToMap(pf._1, names, domains, sliceLen)(arr)))
               Some(arr)
else
               None
}
 ...
 val r = splitByLineBreak.zipWithIndex.map{
            case (arr, idx) => val row = cnvFromFixedWithIndex(namesWithIdx, domainsWithIdx, sliceLenWithIdx)(arr, idx)
...
context.createDataFrame(r, addStructType(Converter.makeSchemaWithRecordError(nameListWithIdx)))
         ...
         {
         val r = splitByLineBreak.map{ arr => val row = cnvFromFixed(names, domains, sliceLen)(arr)
addArr(Row(row.toSeq :+ (arr.size != totalLen_).toString :_*), arr)
}

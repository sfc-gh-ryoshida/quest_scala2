...
/*EWI: SPRKSCL1142 => org.apache.spark.sql.catalyst.util.CaseInsensitiveMap is not supported*/
import org.apache.spark.sql.catalyst.util.CaseInsensitiveMap
import com.snowflake.snowpark_extensions.Extensions._
import com.snowflake.snowpark_extensions.Extensions.functions._
case class InputArgs (d2kBasePath: String, productionMode: String, confPath: String, dataPath: String, projectId: String, processId: String, applicationId: String, runningDateFileFullPath: String, confBasePath: String, fileConvInputFile: String, fileConvOutputFile: String, baseInputFilePath: String, baseOutputFilePath: String, baseErrProofFilePath: String, inputRunningDates: Array[String], inRunningSqlDate: Date, isDebug: Boolean = false) extends {
...
if (mapperPath.isEmpty)
            ...
            {
            Source.fromFile(mapperPath).getLines.map{ line => val splited = line.split("\t")
(splited(0), splited(1))
}.toMap
            }
      }.recover{
      case t => Map.empty[String, String]
      }.get
   }
...
"charSet" -> readDbInfo.charSet))
 val ps = JdbcUtils.createConnectionFactory(options)().prepareStatement("select DT_FROMUPDYMDTM, DT_TOUPDYMDTM from MOP012 where ID_TBLID = ?")
...
result.headOption.getOrElse(throw new IllegalArgumentException (s"tableName is not defined[$tableName]"))
 ...
 object InputArgs {
   def apply(d2kBasePath: String, productionMode: String, confPath: String, dataPath: String, projectId: String, processId: String, applicationId: String, runningDateFileFullPath: String) = {

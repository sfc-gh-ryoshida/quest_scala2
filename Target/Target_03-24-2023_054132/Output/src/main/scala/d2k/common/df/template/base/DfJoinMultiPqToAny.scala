...
joinPqInfoList.foldLeft(orgDf){(odf, pqInfo) => val pqDf = new PqCtl (inFilePath).readParquet(pqInfo.name)
 val pname = if (pqInfo.prefixName.isEmpty)
         pqInfo.name
else
         pqInfo.prefixName
 val addNameDf = pqDf.columns.foldLeft(pqDf){(df, name) =>df.withColumnRenamed(name, s"$pname#$name")
}

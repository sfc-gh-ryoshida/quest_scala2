...
domain match {
 ...
 def execArrayByte(domain: String, data: Array[Byte], charEnc: String) : Either[String, String] = data match {
      case targetif domain == "Byte配列" => Right(new String (target, "ISO-8859-1"))
      case targetif (isPd(domain) || isZd(domain)) && (isNull(target)) => exec(domain.dropRight(3), new String (data, charEnc))
      case targetif (isPd(domain) || isZd(domain)) && (isEmpty(target)) => digitErrOrConvDate(domain.dropRight(3), new String (data, charEnc))
      case targetif isPd(domain) && !isValidSign(target) => digitErrOrConvDate(domain.dropRight(3), new String (data, charEnc))
      case targetif isPd(domain) => convDigitOrDatePd(domain.dropRight(3), target)
      case targetif isZd(domain) && (!isValidSignZd(target)) => digitErrOrConvDate(domain.dropRight(3), new String (data, charEnc))
      case targetif isZd(domain) => convZd(domain.dropRight(3), target)
      case targetif JefConverter.isJefHalf(domain, charEnc) => exec(domain, JefConverter.convJefToUtfHalf(data))
      case targetif JefConverter.isJefFull(domain, charEnc) => exec(domain, JefConverter.convJefToUtfFull(data))
      case _ => exec(domain, new String (data, charEnc))
   }
 def dp(proc: =>Either[String, String]) = Try(proc).recover{
   ...
   */
   def convDate(data: String, min: String, max: String, format: DateTimeFormatter) = data match {
      case targetif (isNull(target)) => Right(min)
      case targetif (isEmpty(target)) => Right(min)
      case targetif (!isDigit(target)) => Right(min)
      case allNineRegex(_*) => Right(max)
      case targetif (target.toLong < min.toLong) => Right(min)
      case targetif (target.toLong > max.toLong) => Right(min)
      case targetif (!isDate(target, format)) => Right(min)
...
data match {
         ...
         case targetif (!isDigit(target)) => Right(min)
         case allNineRegex(_*) => Right(max)
         case targetif (targetMinPad.toLong <= min.toLong) => Right(min)
         case targetif (targetMaxPad.toLong > max.toLong) => Right(min)
         case targetif (!isDate(target, formatter)) => Right(min)
   ...
   */
   def convDateParts(data: String, min: String, max: String) = data match {
      ...
      case targetif (!isDigit(target)) => Right(min)
      case allNineRegex(_*) => Right(max)
      case targetif (target.toInt < min.toInt) => Right(min)
      case targetif (target.toInt > max.toInt) => Right(min)
      case target => Right(target)
   }
   ...
   */
   def convTime(data: String, min: String, max: String) = data match {
      ...
      case targetif (target.toInt < min.toInt) => Right(min)
      case targetif (target.toInt > max.toInt) => Right(min)
      case targetif (!isDate(target.drop(2), FORMATTER_MMSS)) => Right(min)
      case target => Right(target)
   }
...
exec(domain, padForDate(domain, unpacked))}
 ...
 def digitErrOrConvDate(orgDomain: String, data: String) : Either[String, String] = orgDomain match {
 ...
 def convCommMthd(data: String, allSpace: String) = data match {
      case targetif (isNull(target)) => Right("")
      case targetif (isEmpty(target)) => Right("")
      case _ => Right(data.trim().substring(0, 1))
   }
 def convSignedDigit(data: String) = {
   val default = "0"
 val num = if (data.isEmpty)
         {
         default
         }
else
...
(if (isMinus)
 ...
 def unpackForId(data: Array[Byte]) = data.foldLeft(""){(l, r) =>l + f"$r%02x"}.dropRight(1)
 def zeroPadLeft(target: String, fullLen: Int) = s"${ "0" * (fullLen - target.length()) }${ target }"
 def isDate(target: String, format: DateTimeFormatter) = Try(format.withZoneUTC.parseDateTime(target)).map(_ =>true).getOrElse(false)
 ...
 def convDigit(data: String) = data match {
      case targetif (isNull(target.getBytes("MS932"))) => Right(target)
      case targetif (!isDigit(target.trim())) => Left(ERR_MSG_INVALID_VALUE)
      case targetif (isEmpty(target)) => Left(ERR_MSG_INVALID_VALUE)
      case target => Right(target)
   }
...
if (isMinus)
 ...
 def unzoneForId(data: Array[Byte]) = data.map( x =>f"$x%02x".drop(1)).mkString
}
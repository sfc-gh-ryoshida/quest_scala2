...
import com.snowflake.snowpark_extensions.Extensions.functions._
 object PostCodeConverter {
   val postDataMap = new PqToVal[Map[String, (String, String)]] with Nothing {
      val componentId = "MBA935.pq"

      def outputValue(rows: Array[Row]) : Map[String, (String, String)] = {
      rows.map{ row =>(row.getAs[String]("CD_ZIP7LEN").trim, (row.getAs[String]("CD_KENCD"), row.getAs[String]("CD_DEMEGRPCD")))
}.toMap
      }
   }
 ...
 class PostCodeConverter (implicit inArgs: InputArgs) extends Serializable {
   ...
   def localGovernmentCode(postCodeName1: String, postCodeName2: String = "")(outName1: String, outName2: String = "") = cnvLocalGovernmentCode(postCodeName1, postCodeName2)(outName1, outName2)

   private [this] def cnvLocalGovernmentCode(postCodeName1: String, postCodeName2: String)(outName1: String, outName2: String) = (df: DataFrame) =>{
   val convertUdf = udf{(inPostCode: String) => val postCode = Option(inPostCode).map(_.replaceAllLiterally("-", "").trim).getOrElse("")
...
(df2 ~> editColumns(outCol)).drop("_POSTCODES_").na.fill("", Seq(outName1, outName2)).na.replace(outName1, Map("" -> "99")).na.replace(outName2, Map("" -> "999"))
   }
}
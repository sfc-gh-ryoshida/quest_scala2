...
import com.snowflake.snowpark_extensions.Extensions.functions._
 object KanaConverter extends Serializable {
   ...
   private [this] val zenkakuCnv = (inStr: String) =>{
   if (inStr.getBytes(charEnc).size >= 2)
         {
         "*"
         }
else
   ...
   def apply(inStr: String) = {
   if (inStr != null)
         {
         inStr.map( c =>cnvMap.getOrElse(c.toString, zenkakuCnv(c.toString))).mkString
         }
else
         {
         inStr
   ...
   def select(inStr: String) = {
   if (inStr != null)
         {
         inStr.map( c =>cnvMapSelect.getOrElse(c.toString, zenkakuCnv(c.toString))).mkString
         }
else
         {
         inStr

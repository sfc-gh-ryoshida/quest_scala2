...
val dirPath = fileInputInfo.inputDir(componentId)
 ...
 val itemConfId = if (fileInputInfo.itemConfId.isEmpty)
      itemConfIdTemplate
else
      fileInputInfo.itemConfId
 ...
 val extractTargetsForSqlFilter = itemConfs.filter(_.extractTarget).map( item =>s"substring(${ item.itemId },1,8) = ${ inArgs.runningDateYMD }").mkString(" or ")
...
fi.withIndex, fi.preFilter, if (fi.withBinaryRecord.isEmpty)
               None
else
               Some(fi.withBinaryRecord))

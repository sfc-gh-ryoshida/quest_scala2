...
import com.snowflake.snowpark_extensions.Extensions.functions._
 object MakeDate {
   def timestamp_yyyyMMdd(str: String) = Timestamp.valueOf(LocalDateTime.of(str.take(4).toInt, str.drop(4).take(2).toInt, str.drop(6).take(2).toInt, 0, 0, 0))

   def timestamp_yyyyMMddhhmmss(str: String) = Timestamp.valueOf(LocalDateTime.of(
...
str.drop(8).take(2).toInt, str.drop(10).take(2).toInt, str.drop(12).take(2).toInt))
   ...
   def timestamp_yyyyMMddhhmmssSSS(str: String) = Timestamp.valueOf(LocalDateTime.of(
str.take(4).toInt, str.drop(4).take(2).toInt, str.drop(6).take(2).toInt, 

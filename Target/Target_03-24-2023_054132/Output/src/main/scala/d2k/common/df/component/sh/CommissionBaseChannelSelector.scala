...
import com.snowflake.snowpark_extensions.Extensions.functions._
 object CommissionBaseChannelSelector {
   ...
   def comm試算(uniqueKeys: String*) = new trComm試算 {
      override val groupingKeys = uniqueKeys
   }
   ...
   def comm実績_月次手数料(uniqueKeys: String*) = new trComm実績_月次手数料 {
      override val groupingKeys = uniqueKeys
   }
   ...
   def comm実績_割賦充当(uniqueKeys: String*) = new trComm実績_割賦充当 {
      override val groupingKeys = uniqueKeys
   }
   ...
   def comm実績_直営店(uniqueKeys: String*) = new trComm実績_直営店 {
      override val groupingKeys = uniqueKeys
   }
   ...
   def comm毎月割一時金(uniqueKeys: String*) = new trComm毎月割一時金 {
      override val groupingKeys = uniqueKeys
   }
}
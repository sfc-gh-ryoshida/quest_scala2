...
def makeDf(options: Map[String, String])(names: Seq[String], domains: Seq[String], path: Set[String]) = {
   val rdd = SparkContexts.context.read.options(options).csv(path.toSeq :_*).rdd.map{ row => val dataAndDomainsAndNames = row.toSeq.map( s =>Option(s).map(_.toString).getOrElse("")).zip(domains).zip(names)
